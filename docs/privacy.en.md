...
Analysis does not run automatically on visited threads. After the user opens a thread's analysis side panel, the extension collects that thread for review if no local report exists and a configured model key is available. Collection can also be started manually and makes no model requests. Only after the user reviews the sending scope and budget and deliberately starts analysis are thread text, original links, extracted claims, verification questions, and selected source text sent to the configured model API. Usernames and mentions may remain in the original text; display aliases are not full anonymization. Site cookies, login credentials, editor drafts, and global browsing history are not sent.
